...
require_relative 'MultifonClient_types'
...
class MultifonClientSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = MultifonClientHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MultifonClientHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, MultifonClientFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    MultifonClientUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = MultifonClientHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = MultifonClientHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = MultifonClientHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
    ...
    ctx.spec = MultifonClientSpec.new({
  ...
  def raw_request(fetchargs = {})
    ...
    rescue MultifonClientError => err
    ...
    if fetched.is_a?(Hash)
      status = MultifonClientHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MultifonClientError.new(
        "graphql_error", "MultifonClientSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.AccountManagement.list / client.AccountManagement.load({ "id" => ... })
  def AccountManagement(data = nil)
    require_relative 'entity/account_management_entity'
    AccountManagementEntity.new(self, data)
  end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = MultifonClientSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
