...
The Ruby SDK for the MultifonClient API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.AccountManagement(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.MultifonClient(nil).load(
  { "id" => "test01" }, nil
)
