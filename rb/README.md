# MultifonClient Ruby SDK



The Ruby SDK for the MultifonClient API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.AccountManagement` — with named operations (`load`/`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = MultifonClientSDK.test(testopts, sdkopts)
```
...
raise a `MultifonClientError` (a `StandardError` subclass), so wrap
...
```
rb/
├── MultifonClient_sdk.rb       -- Main SDK module
...
The main module (`MultifonClient_sdk`) exports the SDK class
