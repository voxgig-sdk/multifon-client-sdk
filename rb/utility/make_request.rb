...
module MultifonClientUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MultifonClientResponse.new({})
    ...
    if fetch_err
      ...
      response = MultifonClientResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MultifonClientResponse.new(fetched)
    else
