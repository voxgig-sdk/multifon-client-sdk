...
local MultifonClientSDK = {}
MultifonClientSDK.__index = MultifonClientSDK
...
MultifonClientSDK._make_feature = _make_feature


function MultifonClientSDK.new(options)
  local self = setmetatable({}, MultifonClientSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function MultifonClientSDK:options_map()
...
function MultifonClientSDK:get_utility()
  return Utility.copy(self._utility)
end


function MultifonClientSDK:get_root_ctx()
  return self._rootctx
end


function MultifonClientSDK:prepare(fetchargs)
...
function MultifonClientSDK:direct(fetchargs)
...
-- by passing it.
function MultifonClientSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function MultifonClientSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "MultifonClientSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:AccountManagement():list() / client:AccountManagement():load({ id = ... })
...
function MultifonClientSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = MultifonClientSDK.new(sdkopts)
  sdk.mode = "test"
