...
Check `err` first (it is non-`nil` on failure), then use `value`:
    ...
    local account_management, err = client:AccountManagement():load({ id = "example_id" })
    if err then error(err) end
...
```lua
local account_management, err = client:AccountManagement():load({ id = "account_management_id" })
```
