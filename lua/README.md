# MultifonClient Lua SDK



The Lua SDK for the MultifonClient API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:AccountManagement()` — each with the same small set of operations (`load`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/multifon-client-sdk/releases)),
...
Check `err` first (it is non-`nil` on failure), then use `value`:
    ...
    if err then error(err) end
    -- account_management is the loaded record
...
Create an instance: `local account_management = client:AccountManagement(nil)`
...
```
lua/
├── multifon-client_sdk.lua    -- Main SDK module
...
The main module (`multifon-client_sdk`) exports the SDK constructor
