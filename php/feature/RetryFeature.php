...
// MultifonClient SDK retry feature
...
class MultifonClientRetryFeature extends MultifonClientBaseFeature
{
    ...
    public function init(MultifonClientContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MultifonClientContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MultifonClientContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MultifonClientContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
