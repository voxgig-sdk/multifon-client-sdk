...
class MultifonClientSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = MultifonClientHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = MultifonClientHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!MultifonClientFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, MultifonClientFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return MultifonClientUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = MultifonClientHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = MultifonClientHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = MultifonClientHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new MultifonClientError($op . "_allow",
                "MultifonClientSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = MultifonClientHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new MultifonClientError("graphql_error",
                "MultifonClientSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_account_management = null;

    // Canonical facade: $client->AccountManagement()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->account_management()
    // resolves here too.
    public function AccountManagement($data = null)
    {
        require_once __DIR__ . '/entity/account_management_entity.php';
        if ($data === null) {
            if ($this->_account_management === null) {
                $this->_account_management = new AccountManagementEntity($this, null);
            }
            return $this->_account_management;
        }
        return new AccountManagementEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new MultifonClientSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
