# MultifonClient PHP SDK Reference

Complete API reference for the MultifonClient PHP SDK.
...
#### `MultifonClientSDK::test($testopts = null, $sdkopts = null)`
...
#### `AccountManagement($data = null)`

Create a new `AccountManagementEntity` instance. Pass `null` for no initial data.
...
```php
[$result, $err] = $client->AccountManagement()->load(["id" => "account_management_id"]);
```
