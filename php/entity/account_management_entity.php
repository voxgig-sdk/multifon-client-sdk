...
// MultifonClient SDK AccountManagement entity
...
class AccountManagementEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new AccountManagementEntity($this->_client, $opts);
    }

    /**
     * @param AccountManagement|array $args AccountManagement data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = MultifonClientHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return AccountManagement|array The current AccountManagement data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of AccountManagement fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = MultifonClientHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of AccountManagement fields).
     */
    ...
    /**
     * Load a single AccountManagement.
     *
     * @param AccountManagementLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed AccountManagementLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return AccountManagement|array The loaded AccountManagement as an assoc-array at the
     *   SDK boundary; throws MultifonClientError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = MultifonClientHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
