...
class ApiEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MULTIFONCLIENT_TEST_API_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function api_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("MULTIFONCLIENT_TEST_API_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["MULTIFONCLIENT_TEST_API_ENTID"]);
