...
boru-driven command-line client **and** interactive REPL for the MultifonClient
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/multifon-client-cli)
make build

# 2. See usage (words, entities, env vars)
./multifon-client-cli --help

# 3. Provide credentials once, via the environment
export MULTIFON_CLIENT_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./multifon-client-cli load 1 account_management            # {id:1} shorthand
./multifon-client-cli load '{id:1}' account_management       # explicit match map

# 5. Override the API base URL for a single call
MULTIFON_CLIENT_BASE=https://api.example.com ./multifon-client-cli load 1 account_management

# 6. No arguments -> interactive REPL
./multifon-client-cli
multifon-client> load 1 account_management
multifon-client> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/multifon-client-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/multifon-client-cli load 1 account_management
   ```
...
```sh
./multifon-client-cli load 1 account_management          # scalar shorthand for {id:1}
./multifon-client-cli load '{id:1}' account_management     # explicit match map
```
...
./multifon-client-cli load 1 account_management
```
...
Run with no arguments to open a REPL (prompt `multifon-client>`). Each line is
evaluated as its own boru expression:

```text
$ ./multifon-client-cli
multifon-client> load 1 account_management
multifon-client> /help
multifon-client> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `account_management`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/multifon-client-cli`. |
...
(`./multifon-client-cli <expr>`) and interactively (the REPL), and expressions compose the
