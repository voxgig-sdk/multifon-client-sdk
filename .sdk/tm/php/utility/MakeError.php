...
class MultifonClientMakeError
{
    public static function call(?MultifonClientContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MultifonClientContext([], null);
        }
        $op = $ctx->op ?? new MultifonClientOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MultifonClientResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MultifonClientError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MultifonClientError('', $msg, $ctx);
        ...
        if ($err instanceof MultifonClientError) {
