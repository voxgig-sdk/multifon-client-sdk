...
// MultifonClient SDK feature test
//
...
class FtCtrl extends MultifonClientControl
{
...
class FtClient
{
    ...
    private MultifonClientUtility $utility;

    public function __construct(MultifonClientUtility $utility, array $options)
    {
    ...
    public function get_utility(): MultifonClientUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MultifonClientContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MultifonClientContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MultifonClientContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MultifonClientConfig::make_feature($name);
    ...
    private static function buildUrl(MultifonClientSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MultifonClientError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MultifonClientResponse($fetched);
        }
    ...
    private function fail(MultifonClientContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MultifonClientContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
