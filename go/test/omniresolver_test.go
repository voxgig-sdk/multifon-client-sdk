...
import (
	...
	omni "github.com/voxgig-sdk/multifon-client-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MultifonClientError); is {
				out := map[string]any{
					"name":    "MultifonClientError",
...
func omniCtx(arg any, client *sdk.MultifonClientSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MultifonClientSDK); is {
			client = live
