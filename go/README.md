# MultifonClient Golang SDK



The Golang SDK for the MultifonClient API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.

It exposes the API as capitalised, semantic **Entities** — e.g. `client.AccountManagement(nil)` — each with the same small set of operations (`Load`, `Create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
func main() {
    ...
    accountManagement, err := client.AccountManagement(nil).Load(map[string]any{"method": "example_method"}, nil)
    if err != nil {
        panic(err)
    }
...
```go
accountmanagement, err := client.AccountManagement(nil).Load(map[string]any{"method": "example"}, nil)
if err != nil {
    // handle err
    return
}
...
accountManagement, err := client.AccountManagement(nil).Load(
    map[string]any{"method": "example"}, nil,
)
if err != nil {
    panic(err)
}
...
### NewMultifonClientSDK

```go
func NewMultifonClientSDK(options map[string]any) *MultifonClientSDK
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *MultifonClientSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### MultifonClientSDK methods
...
All entities implement the `MultifonClientEntity` interface.
...
```go
accountManagement, err := client.AccountManagement(nil).Load(map[string]any{"method": "method"}, nil)
if err != nil {
    panic(err)
}
...
```
github.com/voxgig-sdk/multifon-client-sdk/go/
├── multifon-client.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/multifon-client-sdk/go`) re-exports everything needed
...
accountmanagement.Load(map[string]any{"method": "example"}, nil)
