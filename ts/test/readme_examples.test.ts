...
//     (\`new MultifonClientSDK(...)\` / \`MultifonClientSDK.test(...)\` become
//     \`MultifonClientSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'MultifonClientSDK'
...
const TEST_SEED = {"entity":{"account_management":{"test01":{"id":"test01"}},"api":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new MultifonClientSDK(...)\` and any
// \`MultifonClientSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MultifonClientSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MultifonClientSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
