...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.MULTIFONCLIENT_TEST_LIVE ||
    'TRUE' === process.env.MULTIFONCLIENT_TEST_OVERRIDE
  ) {
  ...
  m.MULTIFONCLIENT_TEST_EXPLAIN = process.env.MULTIFONCLIENT_TEST_EXPLAIN || m.MULTIFONCLIENT_TEST_EXPLAIN
