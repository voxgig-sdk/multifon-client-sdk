# MultifonClient TypeScript SDK



The TypeScript SDK for the MultifonClient API — a type-safe, entity-oriented client with full async/await support.
...
```ts
try {
  const accountmanagement = await client.AccountManagement().load({ id: 'example_id' })
...
```ts
const client = new MultifonClientSDK({ apikey: '...' })
...
| `tester(testopts?, sdkopts?)` | `MultifonClientSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `MultifonClientSDK.test(testopts?, sdkopts?)` | `MultifonClientSDK` | Create a test-mode client. |
...
| `client` | `client(): MultifonClientSDK` | Return the parent SDK client. |
...
multifon-client/
├── src/
│   ├── MultifonClientSDK.ts        # Main SDK class
...
await accountmanagement.load({ id: "example_id" })

// accountmanagement.data() now returns the loaded accountmanagement data
// accountmanagement.match() returns { id: "example_id" }
```
