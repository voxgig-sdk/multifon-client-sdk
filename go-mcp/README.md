...
[MCP](https://modelcontextprotocol.io) server exposing the MultifonClient SDK as
two agent tools — `multifon-client_list` and `multifon-client_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/multifon-client-mcp)
make build

# 2. Provide credentials via the environment
export MULTIFON_CLIENT_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user multifon-client \
  -- /absolute/path/to/multifon-client-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./multifon-client-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// multifon-client_list: first page of records
{ "entity": "account_management" }
{ "entity": "account_management", "query": { } }

// multifon-client_load: one record by id
{ "entity": "account_management", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/multifon-client-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user multifon-client \
     -- "$PWD"/dist/*/multifon-client-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `multifon-client_list` and `multifon-client_load` tools now appear
   in new sessions. Ask the agent to *"list account_management using multifon-client"*
   and it calls `multifon-client_list` with `{"entity":"account_management"}`.
...
```sh
export MULTIFON_CLIENT_APIKEY=sk_live_xxx            # API key
export MULTIFON_CLIENT_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `multifon-client_list` tool
...
### Call the `multifon-client_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `multifon-client_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `multifon-client_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MULTIFON_CLIENT_APIKEY` | API key sent with every request. |
| `MULTIFON_CLIENT_BASE` | Optional override of the API base URL. |
...
```sh
./multifon-client-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"multifon-client_load","arguments":{"entity":"account_management","query":{"id":1}}}}'
```
