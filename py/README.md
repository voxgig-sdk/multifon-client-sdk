...
The Python SDK for the MultifonClient API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.AccountManagement(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.MultifonClient(None).load(
    {"id": "test01"}, None
)
