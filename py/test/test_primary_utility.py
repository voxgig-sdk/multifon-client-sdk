...
from multifonclient_sdk.core.spec import MultifonClientSpec
from multifonclient_sdk.core.result import MultifonClientResult
from multifonclient_sdk.core.response import MultifonClientResponse
from multifonclient_sdk.core.operation import MultifonClientOperation
from multifonclient_sdk.core.error import MultifonClientError
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return MultifonClientError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_done_basic(self):
        ...
        def subject(entry):
            ...
            except MultifonClientError as e:
                return None, e
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = MultifonClientResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = MultifonClientSDK.test(None, None)
        ...
        class TestInitFeature(MultifonClientBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(MultifonClientBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = MultifonClientSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = MultifonClientSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = MultifonClientSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = MultifonClientOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = MultifonClientSDK.test(None, setup_opts)
