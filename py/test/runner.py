# MultifonClient SDK test runner
...
class MultifonClientTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MultifonClientTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MultifonClientTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MultifonClientTestRunner.getenv("MULTIFON_CLIENT_TEST_LIVE")
        override = MultifonClientTestRunner.getenv("MULTIFON_CLIENT_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MultifonClientTestRunner.getenv(key)
        ...
        explain = MultifonClientTestRunner.getenv("MULTIFON_CLIENT_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if MultifonClientTestRunner._test_control is not None:
            return MultifonClientTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MultifonClientTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MultifonClientTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MultifonClientTestRunner.load_test_control()
...
def load_env_local():
    MultifonClientTestRunner.load_env_local()


def env_override(m):
    return MultifonClientTestRunner.env_override(m)


def entity_data(v):
    return MultifonClientTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MultifonClientTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MultifonClientTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MultifonClientTestRunner.load_test_control()


def live_client_options():
    return MultifonClientTestRunner.live_client_options()


def live_delay_ms():
    return MultifonClientTestRunner.live_delay_ms()
